...
To ensure security, the backend container runs under the unprivileged `nobody` user by default (defined in the [Dockerfile](../backend/Dockerfile)).
...
1. In [docker-compose.yml](../docker-compose.yml), the `api` service is configured with `user: "${UID:-1000}:${GID:-1000}"`. This maps the container process to your local host user's UID and GID so that files written to `./data` remain owned by you.
...
This orchestrator prepares the run inside Docker, renders the image on the host, validates metadata, and finalizes the run.

For the full technical reference of all CLI flags, custom templates, daemon mode, Vision API details, and log locations, refer to the dedicated [dailyfx-agent documentation](dailyfx-agent.md).
