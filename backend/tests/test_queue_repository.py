...
def test_queue_fetching_priority_and_starvation():
    ...
    t1 = GenerationTaskModel(
        task_id="starved", status="queued", priority="normal", queued_at=now - timedelta(minutes=35)
    )
    # Task 2: high priority, recent (1 minute ago)
    t2 = GenerationTaskModel(
        task_id="high-recent", status="queued", priority="high", queued_at=now - timedelta(minutes=1)
    )
    db.add_all([t1, t2])
    db.commit()
