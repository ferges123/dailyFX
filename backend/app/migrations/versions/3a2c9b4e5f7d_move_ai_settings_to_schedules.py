...
def upgrade() -> None:
    ...
    with op.batch_alter_table('schedules', schema=None) as batch_op:
        batch_op.add_column(sa.Column('ai_vision_provider', sa.String(length=50), nullable=False, server_default='none'))
        batch_op.add_column(sa.Column('ai_vision_model', sa.String(length=100), nullable=False, server_default='gpt-4o-mini'))
        batch_op.add_column(sa.Column('ai_image_provider', sa.String(length=50), nullable=False, server_default='none'))
        batch_op.add_column(sa.Column('ai_image_model', sa.String(length=100), nullable=False, server_default='gpt-image-1'))
    ...
    try:
        settings_row = connection.execute(
            sa.text("SELECT default_ai_provider, default_ai_model, ai_image_provider, ai_image_model FROM settings LIMIT 1")
        ).fetchone()
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('settings', schema=None) as batch_op:
        batch_op.add_column(sa.Column('default_ai_provider', sa.String(length=50), nullable=False, server_default='none'))
        batch_op.add_column(sa.Column('default_ai_model', sa.String(length=100), nullable=False, server_default='gpt-image-1'))
        batch_op.add_column(sa.Column('ai_image_provider', sa.String(length=50), nullable=True, server_default='none'))
        batch_op.add_column(sa.Column('ai_image_model', sa.String(length=100), nullable=True, server_default='gpt-image-1'))

    # 2. Data Migration: copy settings fields from first schedule if possible
    connection = op.get_bind()
    try:
        schedule_row = connection.execute(
            sa.text("SELECT ai_vision_provider, ai_vision_model, ai_image_provider, ai_image_model FROM schedules LIMIT 1")
        ).fetchone()
