...
def upgrade() -> None:
    ...
    res = conn.execute(sa.text("SELECT id, notification_preset_id FROM schedules WHERE notification_preset_id IS NOT NULL")).fetchall()
    for row in res:
        conn.execute(
            sa.text("INSERT INTO schedule_notification_presets (schedule_id, notification_preset_id) VALUES (:schedule_id, :preset_id)"),
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('schedules', schema=None) as batch_op:
        ...
        batch_op.create_foreign_key('fk_schedules_notification_preset_id', 'notification_presets', ['notification_preset_id'], ['id'])

    # 2. Migrate data back (take the first notification preset from the association table for each schedule)
    conn = op.get_bind()
    res = conn.execute(sa.text("SELECT schedule_id, notification_preset_id FROM schedule_notification_presets")).fetchall()
