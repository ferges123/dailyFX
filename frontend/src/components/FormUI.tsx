...
export function SectionCard({
  ...
  return (
    <section
      className={`flex flex-col gap-2.5 md:gap-3 rounded-xl md:rounded-2xl border border-stone-200/80 bg-white/85 p-3 md:p-4 shadow-[0_8px_24px_rgba(36,29,16,0.04)] backdrop-blur-md ${className}`}
    >
...
export function EmptyState({
  ...
  return (
    <div
      className={`rounded-xl md:rounded-2xl border border-dashed border-stone-200/90 bg-white/60 px-3 py-5 md:px-4 md:py-7 text-center shadow-[0_8px_24px_rgba(36,29,16,0.03)] backdrop-blur-sm ${className}`}
    >
...
export function InlineError({
  ...
  return (
    <div
      className={`flex items-start gap-1.5 md:gap-2 rounded-lg md:rounded-xl border border-rose-200/70 bg-rose-50/80 px-2.5 py-2 md:px-3 md:py-2.5 text-sm text-rose-950 shadow-[0_8px_20px_rgba(180,35,24,0.05)] ${className}`}
    >
...
export function ProviderModelField({
  ...
  const shouldUseFreeText =
    provider !== 'none' &&
    (freeTextProviders.includes(provider) || isCustomMode);
  ...
  return (
    <div
      className={`grid gap-2.5 md:gap-3 rounded-xl md:rounded-2xl border border-stone-200/80 bg-white/85 p-3 md:p-4 shadow-[0_8px_24px_rgba(36,29,16,0.04)] backdrop-blur-md ${className}`}
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        {provider === 'none' ? (
          <Field
            ...
            value=""
            disabled
            placeholder="Enable a provider first"
            optional
          />
        ) : shouldUseFreeText ? (
          <Field
