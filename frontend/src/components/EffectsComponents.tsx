...
export function FilterRow({
  ...
  return (
    <tr
      className={`group transition-colors duration-150 ${enabled ? 'bg-emerald-50/30 font-medium' : 'hover:bg-stone-50/50'}`}
    >
      ...
      {schema.map((field) => {
        ...
        if (field.type === 'number') {
          ...
          return (
            <label
              key={field.key}
              className="flex items-center justify-between gap-2 text-xs w-full sm:inline-flex sm:w-auto sm:justify-start sm:gap-1.5"
            >
              <span className="text-stone-500 font-medium sm:font-normal">
                {field.label}
              </span>
              <input
                ...
                onChange={(e) => {
                  ...
                  onChange(
                    field.key,
                    Number.isFinite(v) ? v : (field.default ?? 0),
                  );
                }}
        ...
        if (field.type === 'multiselect') {
          const selected = Array.isArray(value)
            ...
            : Array.isArray(field.default)
              ? field.default.filter((e): e is string => typeof e === 'string')
              : [];
          return (
            <div
              key={field.key}
              className="flex flex-col gap-1.5 w-full mt-0.5"
            >
              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400 sm:hidden">
                {field.label}
              </span>
              <div className="flex flex-wrap gap-1">
                {(field.options ?? []).map((o) => {
                  ...
                  return (
                    <label
                      key={o.value}
                      className={`inline-flex cursor-pointer items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] ${checked ? 'border-emerald-300 bg-emerald-50 text-emerald-900' : 'border-stone-200 bg-white text-stone-700'}`}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
