...
export function normalizePersonFilters(
  candidate: Partial<ImmichAssetSearchFilters> | null | undefined,
): ImmichPersonFilter[] {
  const rawFilters = Array.isArray(candidate?.personFilters)
    ? candidate.personFilters
    : [];
  const normalized = rawFilters
    .map((item) => {
      if (!item || typeof item !== 'object') return null;
      const personId =
        typeof item.personId === 'string' && item.personId.length > 0
          ? item.personId
          : null;
      const mode =
        item.mode === 'obligatory' ||
        item.mode === 'exclude' ||
        item.mode === 'optional'
          ? item.mode
          : 'optional';
  ...
  const legacyCandidate = candidate as Partial<ImmichAssetSearchFilters> & {
    personIds?: unknown;
    personMode?: unknown;
  };
  const legacyPersonIds = Array.isArray(legacyCandidate?.personIds)
    ? legacyCandidate.personIds.filter(
        (v): v is string => typeof v === 'string' && v.length > 0,
      )
    : [];
  const legacyMode =
    legacyCandidate?.personMode === 'obligatory' ||
    legacyCandidate?.personMode === 'exclude' ||
    legacyCandidate?.personMode === 'optional'
      ? legacyCandidate.personMode
      : 'optional';
...
export function normalizeFilters(
  ...
  return {
    albumIds: Array.isArray(candidate?.albumIds)
      ? candidate.albumIds.filter(Boolean)
      : [],
    personFilters: normalizePersonFilters(candidate),
    startDate:
      typeof candidate?.startDate === 'string' && candidate.startDate.length > 0
        ? candidate.startDate
        : '',
    endDate:
      typeof candidate?.endDate === 'string' && candidate.endDate.length > 0
        ? candidate.endDate
        : '',
    mediaType:
      candidate?.mediaType === 'photo' ||
      candidate?.mediaType === 'video' ||
      candidate?.mediaType === 'all'
        ? candidate.mediaType
        : 'all',
  };
}
